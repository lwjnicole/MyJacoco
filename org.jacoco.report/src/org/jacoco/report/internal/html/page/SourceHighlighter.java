...
final class SourceHighlighter {
	...
	public void render(final HTMLElement parent, final ISourceNode source,
		...
		String classPath = ((SourceFileCoverageImpl) source).getPackageName()
				+ "." + source.getName().replaceAll(".java", "");
	...
	private void renderCodeLine(final HTMLElement pre, final String linesrc,
			final ILine line, final int lineNr, final String classPath)
			throws IOException {
		if (CoverageBuilder.classInfos == null
			...
			for (ClassInfo classInfo : CoverageBuilder.classInfos) {
				String tClassPath = classInfo.getPackages() + "."
